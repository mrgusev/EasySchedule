'use strict';
angular.module('security.service', ['ngRoute'])
    .factory('SecurityService', ['$injector', '$q', '$location', '$rootScope',
        function ($injector, $q, $location, $rootScope) {
            console.info("Entering security module");
            ...
            var service = {
                ...
                facebookLogin: function (token) {
                    var $http = $injector.get('$http');
                    $http.post('/api/v1/login', { facebookCode: token }).success(function () {
                        service.authenticate(function () {
                            $location.url(service.fromUrl);
                        });
                    });
                },
                logout: function () {
